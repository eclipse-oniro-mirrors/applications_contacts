...
npm -v
# 签名工具路径
export SIGNTOOL_PATH=$(dirname "$(pwd)")/vendor/huawei/binary/artifacts/publickeyinfrastructure/sign_tool
...
function init_ohpm {
    ...
    # 安装ohpm
    cd ${OHPM_HOME}
    wget --no-check-certificate -q "/artifactory/sz-software-release/devecomanagercloud/release/PackageManagerCLI/${ohpm_version}/ohpm.zip"
    unzip -oq ohpm.zip
...
# 环境适配
function build {
  DIR_NAME=$(ls $HM_SDK_HOME|head -1)
  # 从SDK拷贝签名jar包
  cp ${HM_SDK_HOME}/${DIR_NAME}/openharmony/toolchains/lib/hap-sign-tool.jar sign/hap-sign-tool.jar
   ...
   # 如果构建过程报错 ERR_PNPM_OUTDATED_LOCKFILE，需要增加配置：lockfile=false
    ...
    hvigorw --mode module -p module=entry@ohosTest -p debuggable=false -p isOhosTest=true -p ohos-test-coverage=true assembleHap packageTesting --analyze=normal --parallel --incremental --daemon
    ...
    cp entry/build/default/outputs/default/entry-default-signed.hap entry/build/default/outputs/default/Contacts.hap
}
